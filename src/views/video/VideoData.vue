  ...
  <div v-else class="main-content">
            ...
            <div class="lex-direction-column">
              ...
              <n-space size="large" class="detailButtons">
                <!-- <n-button
                  type="info"
                  :loading="btnLoading"
                  class="detailButton outlineButton"
                  @click="Play()"
                >
                  <template #icon>
                ...
                <button @click="ReNewPlayed()" :class="[data.played ? 'active' : '', 'detailButton', 'outlineButton']">
                  <span class="button-icon">
                ...
                <button v-show="is_admin" @click="deleteModal = !deleteModal" class="detailButton circleButton">
                  <span class="button-icon">
                    <i class='bx bxs-trash-alt'></i>
                  </span>
                </button>
                <n-dropdown v-if="is_admin" trigger="hover" size="large" :options="options" @select="handleSelect">
                    ...
                    <div class="view-item-header">
                      <div class="view-item-tag-list">
                        ...
                        <div class="flex-row">
                          ...
                          <n-badge value="New" v-if="handleShowNewBadge(item.last_air_date)" class="new-badge"></n-badge>
                        </div>
                      </div>
                    </div>
      ...
      <n-card style="width: 600px" title="确认" :bordered="false" size="huge" role="dialog" aria-modal="true">
        ...
        <template #footer>
          <n-space justify="end" size="medium">
            <n-button @click="deleteModal = !deleteModal" type="info" :loading="btnLoading">
              <i class='bx bx-x'></i>
              取消
            </n-button>
            <n-button @click="DeleteVideo()" type="warning" :loading="btnLoading">
              <i class='bx bx-check'></i>
              确定
            </n-button>
          </n-space>
          ...
          <div class="search-list">
            <div class="search-itme" v-for="(item, index) in searchData" :key="index">
              <div class="search-img">
                <img loading="lazy" :src='"https://tmdb-image-prod.b-cdn.net/t/p/w220_and_h330_face/" + item.poster_path'>
              </div>
              <div class="search-content">
                ...
                <n-space justify="end" size="medium">
                  <n-button @click="RefVideo(item.id)" type="info" :loading="btnLoading">
                    选中刮削
                  </n-button>
                </n-space>
              </div>
    ...
    <n-modal transform-origin="center" v-model:show="titleModal">
      <n-card style="width: 600px" title="修改资源" :bordered="false" size="huge" role="dialog" aria-modal="true">
        ...
        <template #footer>
          <n-space justify="end" size="medium">
            <n-button @click="UpdateVideo()" type="info" :loading="btnLoading">
              <template #icon>
...
.detailButton {
  background: hsla(285, 4.2%, 40%, 0.7);
